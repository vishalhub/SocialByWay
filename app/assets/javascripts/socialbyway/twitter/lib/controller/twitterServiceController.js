 ...
 * @constructor
 **/
SBW.Controllers.Services.Twitter = SBW.Controllers.Services.ServiceController.extend(
  /** @lends SBW.Controllers.Services.Twitter# */
  {
    /**
     * @constant
     * @type {string}
    ...
    startActionHandler: function (callback) {
      ...
      if (service.authWindowReference == null || service.authWindowReference.closed) {
        service.authWindowReference = window.open('', 'Twitter' + new Date().getTime(), service.getPopupWindowParams({height: 500, width: 400}));
        ...
        var message = {
          ...
          method: "GET",
          parameters: { oauth_callback: service.callbackUrl}
        };
        var url = service.signAndReturnUrl(this.requestTokenUrl, message);
        if (!service.accessObject.user_id || !service.accessObject.consumerSecret || !service.accessObject.tokenSecret) {
          this.sendTwitterRequest({url: url}, function (response) {
    ...
    getAccessToken: function (callback) {
      ...
      if (twitterVerifier) {
        var message = {
          ...
          method: "GET",
          parameters: { oauth_token: service.accessObject.access_token, oauth_verifier: twitterVerifier}
        };
        ...
        this.sendTwitterRequest({url: url}, function (response) {
          ...
          var user = new SBW.Models.User({name: decodeURIComponent(jsonResp.screen_name), id: service.accessObject.user_id});
    ...
    sendTwitterRequest: function (data, successCallback, errorCallback) {
      var service = this, index, headerLength, header;
      var headers = {};
      if (data.header) {
    ...
    getShareCount: function (url, successCallback, errorCallback) {
      this.getCount({url: url}, function (response) {
        if (typeof response === 'string') {
          response = JSON.parse(response);
        }
        successCallback(response);
    ...
    getCommentsForUrl: function (parameters, successCallback, errorCallback) {
      ...
      this.searchTweets(requestObject, function (response) {
        var sbwObject = [], sbwTweetObject, index;
        var tweets = response.results, tweet;
        for (index in tweets) {
